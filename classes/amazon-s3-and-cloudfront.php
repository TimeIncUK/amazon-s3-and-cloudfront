...
class Amazon_S3_And_CloudFront extends AWS_Plugin_Base {
	...
	private $checkFunctionExists = false;
	...
	/**
	 * Given a media library file, it makes sure that it will be downloaded from S3 and made available to the 
	 * thumbnail generator or the media library editor.
	 * 
	 * @param string $file the path in the wp-content/upload dir file
	 * @param string $attachment_id the file attachment id within the wp media library
	...
	public function get_attached_file( $file, $attachment_id ) {
		...
		if ( ! $this->parent_dir_exists( $file ) ) {
			return $file;
		}
		
		if( ! $this->checkFunctionExists ){
			$this->download_url_exists();
		}
	 ...
	 */
	public function parent_dir_exists( $file ) {
		$exists = true;
	...
	private function download_url_exists(){
		if ( ! function_exists( 'download_url' ) ) {
			require_once ( ABSPATH . 'wp-admin/includes/file.php' );
		}
		$this->checkFunctionExists = true;
	}
	...
	function get_setting( $key ) {
		...
		if ( isset( $settings['wp-uploads'] ) && $settings['wp-uploads'] &&
	...
	function delete_attachment( $post_id ) {
		...
		if ( $objects ) {
			...
			try {
				$this->get_s3client()->deleteObjects( 
					array( 'Bucket' => $s3object['bucket'], 'Objects' => $hidpi_images ) );
			} catch ( Exception $e ) {
			}
		}
		
		$objects[] = array( 'Key' => $s3object['key'] );
		
		try {
			$this->get_s3client()->deleteObjects( 
				array( 'Bucket' => $s3object['bucket'], 'Objects' => $objects ) );
		} catch ( Exception $e ) {
	...
	function wp_generate_attachment_metadata( $data, $post_id ) {
		...
		$acl = apply_filters( 'wps3_upload_acl', 'public-read', $type, $data, $post_id, $this ); // Old naming convention,
		...
		$args = array( 'Bucket' => $bucket, 'Key' => $prefix . $file_name, 'SourceFile' => $file_path, 'ACL' => $acl );
		...
		add_post_meta( $post_id, 'amazonS3_info', array( 'bucket' => $bucket, 'key' => $prefix . $file_name ) );
		
		$additional_images = array();
		
		if ( isset( $data['thumb'] ) && $data['thumb'] ) {
			$path = str_replace( $file_name, $data['thumb'], $file_path );
			$additional_images[] = array( 'Key' => $prefix . $data['thumb'], 'SourceFile' => $path );
			$files_to_remove[] = $path;
		} elseif ( ! empty( $data['sizes'] ) ) {
			foreach ( $data['sizes'] as $size ) {
				$path = str_replace( $file_name, $size['file'], $file_path );
				$additional_images[] = array( 'Key' => $prefix . $size['file'], 'SourceFile' => $path );
				$files_to_remove[] = $path;
			}
		}
	...
	function wp_get_attachment_url( $url, $post_id ) {
		...
		$new_url = apply_filters( 'wps3_get_attachment_url', $new_url, $post_id, $this ); // Old naming convention, will be
		                                                                                // deprecated soon
